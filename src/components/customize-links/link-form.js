...
function LinkForm({ fieldId, index, platform, link }) {
  ...
  const [isFieldDirty, setIsFieldDirty] = useState(isListEdited.current);
  ...
  useEffect(() => {
    setIsFieldDirty(isListEdited.current);
      ...
      >
        ...
        <div className="flex flex-col gap-5">
          ...
          <Input
            ...
            title={"Link"}
            type="text"
            placeholder={`e.g. https://www.${ userPlatform ? String(userPlatform).toLowerCase().split(' ').join('') : "website" }.com/johnappleseed`}
