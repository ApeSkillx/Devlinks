...
import allMenuList from "@/src/lib/all-menu-list";
...
function Dropdown({ name, onChange, onClick, value, error, errorMessage }) {
  ...
  return (
    <div className="py-1 relative">
      ...
      <div
        className={`w-full h-[48px] flex items-center justify-between cursor-pointer bg-white transition-all duration-300 ease-in-out rounded-lg py-3 px-3 border border-neutral-borders hover:border-primary-index hover:shadow-3xl ${dropdownOpen ? "!border-primary-index !shadow-3xl" : ""
          } ${error ? "border-error" : ""}`}
        ...
        <ul className="absolute top-20 right-0 z-50 bg-white shadow-4xl py-3 px-4 w-full max-h-[15rem] rounded-lg border border-neutral-borders overflow-x-hidden overflow-y-scroll">
          {allMenuList.map((item, index, arr) => (
            <li
              ...
              className={`flex items-center gap-3 cursor-pointer border-b-2 border-neutral-borders py-3  ${index === 0 ? "pt-0" : ""
