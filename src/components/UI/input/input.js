...
function Input({
  ...
  return (
    ...
    >
      ...
      <div className={`relative ${profile ? "md:w-7/12" : ""
        }`}>
        <input
          className={`w-full text-neutral-dark-grey ring-neutral-borders ring-1 cursor-pointer bg-white outline-none transition-all duration-300 ease-in-out rounded-lg py-3 ${hasIcon ? "pl-10" : "px-4"} ${error
        ...
        {hasIcon && (
          <span className="absolute top-1/2 left-3 transform -translate-y-1/2">
            {title === "Link" ? (
              ...
              <Image
                src={`/images/icon-${id === "confirm-password" ? "password" : id
                  }.svg`}
                alt={title}
                width="16"
                height="16"
